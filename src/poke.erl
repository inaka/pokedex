...
%% @doc Starts the app
-spec start() -> {ok, [atom()]}.
start() -> application:ensure_all_started(pokedex).

%% @doc Stops the app
-spec stop() -> ok.
stop() -> application:stop(pokedex).
...
start_phase(start_cowboy, _StartType, []) ->
  Port = application:get_env(pokedex, http_port, 8080),
  ListenerCount = application:get_env(pokedex, http_listener_count, 10),
  ...
  case cowboy:start_http(pokedex, ListenerCount, TransOpts, ProtoOpts) of
    {ok, _} -> ok;
